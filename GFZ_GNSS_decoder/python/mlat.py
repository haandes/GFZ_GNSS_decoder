...
def ecef2llh(X):
    (x,y,z)=X
    ep  = np.sqrt((wgs84_a2 - wgs84_b2) / wgs84_b2)
    p   = np.sqrt(x**2+y**2)
    th  = np.arctan2(wgs84_a*z, wgs84_b*p)
    lon = np.arctan2(y, x)
    lat = np.arctan2(z+ep**2*wgs84_b*np.sin(th)**3, p-wgs84_e2*wgs84_a*np.cos(th)**3)
...
def llh2ecef(X):
    ...
    n = lambda x: wgs84_a / np.sqrt(1 - wgs84_e2*(np.sin(x)**2))
    
    x = (n(lat) + alt)*np.cos(lat)*np.cos(lon)
    y = (n(lat) + alt)*np.cos(lat)*np.sin(lon)
    z = (n(lat)*(1-wgs84_e2)+alt)*np.sin(lat)
